...
pub(crate) struct SharedObject {
    ...
    // pub needed_libraries: Vec<&'static str>,
...
impl SharedObject {
    ...
    pub unsafe fn from_program_header_table(
        ...
        let (mut base, mut dynamic_header, mut tls_program_header) = (None, None, None);
        for header in program_header_table {
            match header.p_type {
                PT_PHDR => {
                    base = Some(
                        program_header_table.as_ptr().byte_add(header.p_vaddr) as *const c_void
                    )
                }
        ...
        let (base, dynamic_header, tls_program_header) = (
            base.unwrap(),
            dynamic_header.unwrap(),
            tls_program_header.unwrap(),
        );
        ...
        // syscall_debug_assert!(!symbol_table_pointer.is_null());
